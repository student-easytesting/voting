...
const RegistrationTime = () => {
  ...
  async function calculateUnixTime() {
    ...
    if (startTimeInput && durationInput) {
      ...
      const tx = await contract.setRegistrationTimes(
        startTimeUnix,
        endTimeUnix
      );
      await tx.wait();
  ...
  return (
    <>
      ...
      <form id="timeForm">
        <label htmlFor="startTime">Registration Start Time:</label>
        <input
          type="datetime-local"
          id="startTime"
          name="startTime"
        ...
        <select id="durationUnit" name="durationUnit" className="setinput">
